...
from model.basicsr.utils import img2tensor
...
def test_single(img_gt, img_restored):
    ...
    # calculate lpips
    lpips_val = loss_fn_vgg(img_restored.unsqueeze(0).cuda(), img_gt.unsqueeze(0).cuda()).cpu().data.numpy().flatten()[0]
    return lpips_val

def main():
    # Configurations
    # -------------------------------------------------------------------------
    folder_gt = r'/home/xxx/gt'
    folder_restored = r'/home/xxx/sr'
    suffix = ''
    # -------------------------------------------------------------------------
    loss_fn_vgg = lpips.LPIPS(net='alex').cuda()  # RGB, normalized to [-1,1]
    lpips_all = []
    img_list = sorted(glob.glob(osp.join(folder_gt, '*')))
    print(osp.join(folder_gt, '*'))
